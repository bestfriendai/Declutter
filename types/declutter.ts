...
export type EnergyLevel = 'exhausted' | 'low' | 'moderate' | 'high' | 'hyperfocused';
...
export interface Room {
  ...
  lastCleanedAt?: number; // Timestamp of last task completion in this room
  freshnessDecayRate?: number; // Override default decay rate (points lost per day)
}

// Recurring task for the daily loop system
export interface RecurringTask {
  id: string;
  title: string;
  emoji: string;
  roomId: string;
  frequencyDays: number; // How often this task should recur
...
export interface TaskCluster {
  clusterType: 'proximity' | 'carry_chain' | 'same_category' | string;
  taskIds: string[];
  clusterLabel: string;
  rationale: string;
...
export interface AIAnalysisResult {
  ...
  timeProfiles?: TimeProfiles | null;
  energyProfiles?: EnergyProfiles | null;
